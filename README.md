...
The corridor joiner, intended to be printed in a flexible material such as Flexible PLA (available from rigid.ink).  None of the flexible plastics are UV safe but it's a single replacement component so not a huge risk.  Orient the object so that the corridor sides rise upwards from the heat bed; no supports required and, to give maximum flexiblity, zero infill.  I'm still experimenting with this but current thinking is to print in 0.1 mm detailed resolution and, for this flexible material, no retraction and only 15 mm/s print speed.
